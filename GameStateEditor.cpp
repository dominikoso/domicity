...
void GameStateEditor::update(const float dt)
{
   if(!paused) {
       ...
       this->guiSystem.at("infoBar").setEntryText(1, "PLN " + std::to_string(long(this->city.funds)));
       ...
       this->guiSystem.at("infoBar").setEntryText(4, "Game Paused");
   }
}
        ...
        switch(event.type)
        {
            ...
            {if(!paused) {
                    ...
                    if (this->actionState == ActionState::PANNING) {
                        sf::Vector2f pos = sf::Vector2f(
                                sf::Mouse::getPosition(this->game->window) - this->panningAnchor);
                        ...
                        selectionEnd.x = pos.y / (this->city.map.tileSize) + pos.x / (2 * this->city.map.tileSize) -
                                         this->city.map.width * 0.5 - 0.5;
                        selectionEnd.y = pos.y / (this->city.map.tileSize) - pos.x / (2 * this->city.map.tileSize) +
                                         this->city.map.width * 0.5 + 0.5;
                            ...
                            this->city.map.select(selectionStart, selectionEnd,
                                                  {
                                                          ...
                                                          TileType::INDUSTRIAL, TileType::MOUNTAIN,
                                                          TileType::PYLON
                                                  });
                        }
            ...
            {if (!paused) {
                    ...
                    if (event.mouseButton.button == sf::Mouse::Middle) {
                        ...
                        if (this->guiSystem.at("rightClickMenu").visible == true) {
                            ...
                            if (this->actionState != ActionState::SELECTING) {
                                ...
                                selectionStart.x = gamePos.y / (this->city.map.tileSize) +
                                                   gamePos.x / (2 * this->city.map.tileSize) -
                                                   this->city.map.width * 0.5 - 0.5;
                        ...
                        if (this->actionState == ActionState::SELECTING) {
                            ...
                            if (pos.x >
                                this->game->window.getSize().x - this->guiSystem.at("rightClickMenu").getSize().x) {
                                pos -= sf::Vector2f(this->guiSystem.at("rightClickMenu").getSize().x, 0);
                            }
                            if (pos.y >
                                this->game->window.getSize().y - this->guiSystem.at("rightClickMenu").getSize().y) {
            ...
            case sf::Event::KeyPressed:
            {
                ...
                if (event.type == sf::Event::KeyPressed && event.key.code == sf::Keyboard::F12)
                {
                    ...
                    sf::Image screenshot = this->game->window.capture();
                    std::string format = ".png";
                    std::string date_name = data + format;
                    screenshot.saveToFile(date_name);
                }
    ...
    this->guiSystem.emplace("rightClickMenu", Gui(sf::Vector2f(196, 16), 2, false, this->game->stylesheets.at("button"),
                                                  {
                                                          ...
                                                          std::make_pair("Mountain $" 			+ this->game->tileAtlas["mountain"].getCost(), "mountain"),
                                                          std::make_pair("Forest $" 			+ this->game->tileAtlas["forest"].getCost(), "forest" ),
                                                          std::make_pair("Water $" 			+ this->game->tileAtlas["water"].getCost(), "water"),
                                                          ...
                                                          std::make_pair("Pylon $" + this->game->tileAtlas["pylon"].getCost(), "pylon"),
